...
@Path("/api/player-development-goals")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class PlayerDevelopmentGoalResource {

    private static final String DEFAULT_COLOR = "#8e44ad";

    @GET
    public List<CatalogRefDto> list() {
        return PlayerDevelopmentGoal.listActive().stream().map(CatalogRefDto::from).toList();
    }
